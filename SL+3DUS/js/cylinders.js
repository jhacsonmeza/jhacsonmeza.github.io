...
const container = document.getElementById( 'cylinders' );
...
let pc1;
loader1.load( 'data/cylin_ext.ply', function ( geometry ) {
    ...
    pc1.rotateOnWorldAxis(new THREE.Vector3(0, 1, 0), THREE.Math.degToRad(17));
...
let pc2;
loader2.load( 'data/cylin_int.ply', function ( geometry ) {
    const material2 = new THREE.PointsMaterial( {size: 0.015, vertexColors: true} );
    pc2 = new THREE.Points( geometry, material2 );

    pc2.rotateOnWorldAxis(new THREE.Vector3(0, 1, 0), THREE.Math.degToRad(17));
...
const folder2 = panel.addFolder('Internal structure');

folder1.add( {'show outer cylinder': true}, 'show outer cylinder' ).onChange(function ( visibility ) {
...
folder2.add( {'show inner cylinder': true}, 'show inner cylinder' ).onChange(function ( visibility ) {
